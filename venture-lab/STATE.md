...
- Блокеры спринта (все 4 гейта на старте стоят на approvals): email-секвенция и посты — user-approval по APPROVALS.md; промо-лендинг — approval публикации + плейсмента ($0-варианты не верифицированы); WCMS-метод — NEEDS-CLARIFICATION (аббревиатура и порог «просканированного портфеля» в артефактах не расшифрованы). Без разблокировки полевые цифры не двигаются (0 интервью / 0 сканов / 0 регистраций на день 0).

## Next action
Разблокировать спринт H1: (1) user-approval на email-секвенцию 7 писем S1/S2 и посты-кейсы (APPROVALS.md, APPROVAL REQUIRED в Bot Chat компании), (2) approval публикации промо-лендинга + выбор $0-плейсмента, (3) clarification WCMS-метода и порога «просканированного портфеля», (4) E4-проверка «migrate GPT» в реальном ChatGPT-аккаунте (10 мин). Параллельно: еженедельный kill-мониторинг K1 (проверка №1 — 2026-08-29, не сработал; снапшоты e23–e24) и повторная S2-конкурентная проверка. Полевые цифры → venture-lab/sprint-H1-tracker-2026-08-29.md; агрегация результатов спринта — отдельной карточкой (GO не объявляется на этом уровне).
